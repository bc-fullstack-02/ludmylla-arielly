...
const publischer = Object.keys(defaultConfig.vhosts['/'].publications)[0]
...
module.exports = {
  pub: (req, res, next) => Rascal.Broker.create(Rascal.withDefaultConfig(config), function (err, broker) {
    ...
    req.publish = (type, keys, value) => new Promise((resolve, reject) => {
      ...
      broker.publish(publischer, msg, function (err, publication) {
        if (err) reject(err)
  ...
  sub: () => Promise.resolve(Rascal.withDefaultConfig(config))
    .then((config) => new Promise((resolve, reject) => Rascal.Broker.create(config, (err, broker) => {
      if (err) {
